 ...
 * @subpackage Renderer
 */
class Default_Renderer_Tsjerk_Helper_FormatShip extends Zend_View_Helper_Abstract
{
    ...
    public function formatShip($attacker, Default_Model_Ship $ship, Default_Model_Fleet $fleet = null)
    {
        if (null === $fleet) {
            if ($attacker) {
                return "[color=#ff00ff]{$this->view->translate($ship->getName())} {$this->view->formatNumber($ship->getCount())}[/color]\n";
            } else {
                return "[color=#00ff00]{$this->view->translate($ship->getName())} {$this->view->formatNumber($ship->getCount())}[/color]\n";
            }
        } else {
            /*
